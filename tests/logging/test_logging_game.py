import pytest
from src.game.logging_game import (
    LoggingGame, _HeroSnap, _AllySnap,
    _SnapshotKey, _StatKey, _PhaseKey,
)
...
def _h(name=Heroes.Eowyn, hp=5, max_hp=5, res=0, exhausted=False, sphere=Sphere.Spirit) -> _HeroSnap:
    return _HeroSnap(name, hp, max_hp, res, exhausted, sphere)


def _a(name=Allies.Gondorian_Spearman, hp=2, max_hp=2, exhausted=False) -> _AllySnap:
    return _AllySnap(name, hp, max_hp, exhausted)


def _snap(**overrides) -> dict:
    base = {
        _SnapshotKey.THREAT:              0,
        _SnapshotKey.HEROES:              [],
        _SnapshotKey.BOARD:               [],
        _SnapshotKey.STAGING:             [],
        _SnapshotKey.ENGAGED:             [],
        _SnapshotKey.HAND:                [],
        _SnapshotKey.QUEST_NAME:          None,
        _SnapshotKey.QUEST_PROGRESS:      None,
        _SnapshotKey.QUEST_REQUIRED:      None,
        _SnapshotKey.ACTIVE_LOC:          None,
        _SnapshotKey.ACTIVE_LOC_PROGRESS: None,
        _SnapshotKey.TOTAL_RESOURCES:     0,
    }
...
class TestUpdateStatsResources:
    def test_resources_generated_on_increase(self, game):
        game._update_stats(_snap(total_resources=3), _snap(total_resources=6), _PhaseKey.RESOURCES)
        assert game._stats[_StatKey.RESOURCES_GENERATED] == 3

    def test_resources_generated_accumulates(self, game):
        game._update_stats(_snap(total_resources=0), _snap(total_resources=3), _PhaseKey.RESOURCES)
        game._update_stats(_snap(total_resources=0), _snap(total_resources=3), _PhaseKey.RESOURCES)
        assert game._stats[_StatKey.RESOURCES_GENERATED] == 6

    def test_resources_generated_ignores_decrease(self, game):
        game._update_stats(_snap(total_resources=6), _snap(total_resources=3), _PhaseKey.RESOURCES)
        assert game._stats[_StatKey.RESOURCES_GENERATED] == 0

    def test_resources_generated_only_in_resources_phase(self, game):
        game._update_stats(_snap(total_resources=0), _snap(total_resources=3), _PhaseKey.PLANNING)
        assert game._stats[_StatKey.RESOURCES_GENERATED] == 0

    def test_resources_spent_on_decrease(self, game):
        game._update_stats(_snap(total_resources=6), _snap(total_resources=4), _PhaseKey.PLANNING)
        assert game._stats[_StatKey.RESOURCES_SPENT] == 2

    def test_resources_spent_accumulates(self, game):
        game._update_stats(_snap(total_resources=6), _snap(total_resources=4), _PhaseKey.PLANNING)
        game._update_stats(_snap(total_resources=4), _snap(total_resources=2), _PhaseKey.PLANNING)
        assert game._stats[_StatKey.RESOURCES_SPENT] == 4

    def test_resources_spent_ignores_increase(self, game):
        game._update_stats(_snap(total_resources=3), _snap(total_resources=6), _PhaseKey.PLANNING)
        assert game._stats[_StatKey.RESOURCES_SPENT] == 0

    def test_resources_spent_only_in_planning_phase(self, game):
        game._update_stats(_snap(total_resources=6), _snap(total_resources=4), _PhaseKey.COMBAT)
        assert game._stats[_StatKey.RESOURCES_SPENT] == 0
...
class TestUpdateStatsAllies:
    def test_ally_entering_board_counted(self, game):
        ...
        game._update_stats(before, after, _PhaseKey.PLANNING)
        assert game._stats[_StatKey.ALLIES_PLAYED] == 1
    ...
    def test_ally_leaving_board_not_counted(self, game):
        ...
        game._update_stats(before, after, _PhaseKey.PLANNING)
        assert game._stats[_StatKey.ALLIES_PLAYED] == 0

    def test_multiple_allies_entering_counted(self, game):
        before = _snap(board=[])
        after  = _snap(board=[_a(Allies.Gondorian_Spearman), _a(Allies.Wandering_Took)])
        game._update_stats(before, after, _PhaseKey.PLANNING)
        assert game._stats[_StatKey.ALLIES_PLAYED] == 2

    def test_ally_count_only_in_planning_phase(self, game):
        before = _snap(board=[])
        after  = _snap(board=[_a(Allies.Gondorian_Spearman)])
        game._update_stats(before, after, _PhaseKey.COMBAT)
        assert game._stats[_StatKey.ALLIES_PLAYED] == 0
...
class TestUpdateStatsCombat:
    ...
    def test_damage_to_enemy_tracked(self, game):
        ...
        game._update_stats(before, after, _PhaseKey.COMBAT)
        assert game._stats[_StatKey.DAMAGE_TO_ENEMIES] == 3

    def test_damage_on_enemy_kill_tracked(self, game):
        before = _snap(engaged=[(Enemies.Dol_Guldur_Orcs, 4)])
        after  = _snap(engaged=[])
        game._update_stats(before, after, _PhaseKey.COMBAT)
        assert game._stats[_StatKey.DAMAGE_TO_ENEMIES] == 4
    ...
    def test_combat_stats_only_in_combat_phase(self, game):
        ...
        game._update_stats(before, after, _PhaseKey.QUEST)
        assert game._stats[_StatKey.ENEMIES_DEFEATED] == 0
        assert game._stats[_StatKey.DAMAGE_TO_HEROES] == 0
...
class TestLogSummary:
    ...
    def test_stats_values_in_output(self, game, capsys):
        Logger.enable()
        game._stats[_StatKey.ENEMIES_DEFEATED] = 5
        game._stats[_StatKey.ALLIES_PLAYED] = 3
        game.log_summary(is_victory=True)
